...
def admin_home(request):
    ...
    return render(request, 'trading/admin.html', {'base_role': 'ADMIN'})
def fetch_best_ask():
    ...
    return Order.objects.filter(
        ...
        price__isnull=False,
        is_matched=False,
    ).order_by('price').values('price', 'disclosed').first()
...
def market_maker_home(request):
    ...
    execute_order()
    return render(request, 'trading/market-maker.html', {'user': user, 'orders': orders,'trades': trades,'stoploss_orders': stoploss_orders, 'base_role': user.role})
...
def trader_home(request):
    ...
    execute_order()
    return render(request, 'trading/trader.html', {'user': user, 'orders': orders,'trades': trades,'stoploss_orders': stoploss_orders, 'base_role': user.role})
